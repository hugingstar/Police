...
- '/nfs_k8s/static' (Web용) -> 'nfs-pvc-web'
- '/nfs_k8s/was' (Was용) -> 'nfs-pvc-was'
- '/nfs_k8s/db' (Database용) -> 'nfs-pvc-db'
- '/nfs_k8s/config' (설정파일용) -> 'nfs-pvc-config'
- '/nfs_k8s/monitoring' (모니터링 데이터용) -> 'nfs-pvc-monitoring'
- '/nfs_k8s/airflow' (Airflow 작업용) -> 'nfs-pvc-airflow'
```
## 3. 쿠버네티스 적용 방법
